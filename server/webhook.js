    ...
    embeds: [
      {
        ...
        description: `${username} reached #${rank} on a${
          gameMode == "easy" ? "n" : ""
        } ${_.startCase(gameMode)} Singleplayer game with a score of ${
          finalGameData.currentScore
        } points, killing ${finalGameData.enemiesKilled} out of ${
          finalGameData.enemiesCreated
        } enemies and survived for ${utilities.turnMillisecondsToTime(
          finalGameData.currentInGameTimeInMilliseconds
        )} at ${(
          finalGameData.actionsPerformed /
          (finalGameData.currentInGameTimeInMilliseconds / 60000)
        ).toFixed(3)} actions per minute.`,
