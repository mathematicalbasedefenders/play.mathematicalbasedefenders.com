...
async function giveExperiencePointsToPlayerID(userIDAsString, amount){
    ...
    let oldData = await schemas.getUserModel().findById(userIDAsString, (error2, result2) => {
        if (error2) {
    ...
    await schemas.getUserModel().findByIdAndUpdate(userIDAsString, { $inc: { "statistics.totalExperiencePoints" : amount } }, { upsert: true }, (error3, result3) => {
        if (error3) {
    ...
    if (oldExperiencePointAmount + amount > oldExperiencePointAmount && getLevel(oldExperiencePointAmount + amount) > getLevel(oldExperiencePointAmount)){
