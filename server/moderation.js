...
async function sendReport(reporter, reported, description){
    ...
    console.log(log.addMetadata(`Received a report from ${utilities.getNameOfSocketOwner(reporter)}`,"info"))
...
async function validateReport(reporter){
    ...
    if (!reporter.variables?.userIDOfSocketOwner || !reporter.variables.loggedIn){
        return false;
    }
