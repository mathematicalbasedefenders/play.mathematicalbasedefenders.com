  ...
  if (!usersCurrentlyAttemptingToLogIn.includes(username)) {
    if (/^[a-zA-Z0-9_]*$/g.test(username)) {
      ...
      if (socket.variables.playerDataOfSocketOwner) {
        let result = await checkPassword(username, encodedPassword, socket);
        if (result) {
          // successful login
          ...
          //     })
          // );
          "updateText", "#secondary-top-bar-container", `Level ${playerLevel}`;

          console.log(
            log.addMetadata(`Correct password for ${username}!`, "info")
          );
          return true;
        } else {
          // failed log in
          socket.send(
            JSON.stringify({
              ...
              arguments: {
                title: "Failed log in!",
                text: `Failed to log in as ${username}`
              }
            })
          );
          usersCurrentlyAttemptingToLogIn.splice(
          ...
          console.log(
            log.addMetadata(`Incorrect password for ${username}!`, "info")
          );
          return false;
        }
      } else {
