    ...
    if (roomID.length == 8) {
      ...
      if (roomID == roomIDOfDefaultMultiplayerRoom) {
        if (
          ...
          Object.keys(rooms[roomIDOfDefaultMultiplayerRoom].playersInRoom)
            .length >= 2
        ) {
          ...
          rooms[roomIDOfDefaultMultiplayerRoom].timeToStart = new Date(
            Date.now() +
              (configuration.developerConfiguration.settings.impatientMode
                ? 250
                : 30000)
          );
        } else if (
        ...
        if (
          ...
          let timeLeft =
            rooms[roomIDOfDefaultMultiplayerRoom].timeToStart - Date.now();
          if (timeLeft <= 0) {
            rooms[roomIDOfDefaultMultiplayerRoom].readyToStart = false;
          ...
          global.sockets
            ...
            ?.send(
              JSON.stringify({
                ...
                arguments: {
                  data: rooms[roomID].data.currentGame.players[
          ...
          if (connections) {
            ...
            for (let clientConnectionID of Object.keys(connections)) {
              if (clientConnectionID) {
                if (
                  rooms[roomID].data.currentGame.players[clientConnectionID]
                ) {
                  if (
                    rooms[roomID].data.currentGame.players.hasOwnProperty(
                      clientConnectionID
                    )
                  ) {
                    if (
                      ...
                      try {
                        global.sockets
                          .find(
                            (socket) =>
                              socket.connectionID === clientConnectionID
                          )
                      ...
                      let userIDOfEliminatedPlayer =
                        utilities.getSocketAccordingToPlayerName(
                          rooms[roomID].data.currentGame.players[
                            clientConnectionID
                          ].currentGame.playerName
                        )?.variables.userIDOfSocketOwner || false;
                      ...
                      if (userIDOfEliminatedPlayer) {
                        leveling.giveExperiencePointsToUserID(
                          ...
                          Math.round(
                            rooms[roomID].data.currentGame.players[
                              clientConnectionID
                            ].currentGame.experiencePointsEarned
                          )
                        );
                        utilities
                          ...
                          .send(
                            JSON.stringify({
                              ...
                              arguments: {
                                ...
                                text: `<div><span>(System)</span>: Added ${Math.round(
                                  rooms[roomID].data.currentGame.players[
                                    clientConnectionID
                                  ].currentGame.experiencePointsEarned
                                )} experience points to your account.</div>`,
                                useHTML: true
                              }
                            })
                          );
                      ...
                      rooms[roomID].data.currentGame.ranks.push([
                        [
                          ...
                          rooms[roomID].data.currentGame.players[
                            clientConnectionID
                          ].currentGame.currentInGameTimeInMilliseconds,
                          rooms[roomID].data.currentGame.players[
                            clientConnectionID
                      ...
                      rooms[roomID].data.currentGame.playersAlive.splice(
                        rooms[roomID].data.currentGame.playersAlive.indexOf(
                          clientConnectionID
                        ),
                        1
                      );
                      if (clientConnectionID) {
                        ...
                        if (
                          ...
                          global.sockets
                            .find(
                              (element) =>
                                element.connectionID === clientConnectionID
                            )
                            .send(
                              JSON.stringify({
                                action: "currentGameData",
                        ...
                        let socketToClose = global.sockets.find(
                          (element) =>
                            element.connectionID === clientConnectionID
                        );
                        if (socketToClose) {
                  ...
                  if (
                    Object.keys(rooms[roomID].data.currentGame.players)
                      .length <= 1
                  ) {
                    if (
                      rooms[roomID].data.currentGame.playersAlive.length == 1
                    ) {
                      let winnerSocket = global.sockets.find(
                        (socket) =>
                          socket.connectionID ===
                          rooms[roomID].data.currentGame.playersAlive[0]
                      );
                      let data =
                        constructDefaultMultiplayerGameDataObjectToSend(
                          clientConnectionID
                      ...
                      let userIDOfEliminatedPlayer =
                        winnerSocket?.variables.userIDOfSocketOwner || false;
                      ...
                      if (userIDOfEliminatedPlayer) {
                        leveling.giveExperiencePointsToUserID(
                          ...
                          Math.round(
                            rooms[roomID].data.currentGame.players[
                              winnerSocket.connectionID
                            ].currentGame.experiencePointsEarned * 1.5
                          )
                        );
                        ...
                        if (winnerSocket) {
                          let userID =
                            utilities.getUserIDOfSocketOwnerIfLoggedIn(
                              winnerSocket
                            );
                          if (userID) {
                            User.incrementMultiplayerGamesWonCount(userID);
                          }
                        }
                      }
                        ...
                        JSON.stringify({
                          ...
                          arguments: {
                            ...
                            text: game.formatMultiplayerRoomRanks([
                              rooms[roomID].data.currentGame.ranks
                            ]),
                            useHTML: true
                          }
                        })
...
async function startDefaultMultiplayerGame(roomID) {
  ...
  rooms[roomIDOfDefaultMultiplayerRoom].data.currentGame.globalTileQueues.push(
  ...
  for (let player of players) {
    ...
    if (connectionID) {
      User.incrementMultiplayerGamesPlayedCount(connectionID, 1);
    }
  }
}
...
function constructDefaultMultiplayerGameDataObjectToSend(connection) {
  if (connection) {
    ...
    if (data?.currentGame) {
      if (
        rooms[roomIDOfDefaultMultiplayerRoom]?.playersInRoom[connection]
          ?.variables
      ) {
        data.currentGame.nameColor = utilities.formatPlayerName(
          rooms[roomIDOfDefaultMultiplayerRoom].playersInRoom[connection]
            .variables.playerRank,
          rooms[roomIDOfDefaultMultiplayerRoom].playersInRoom[connection]
            .variables.loggedIn
            ? rooms[roomIDOfDefaultMultiplayerRoom].playersInRoom[connection]
                .variables.usernameOfSocketOwner
            : rooms[roomIDOfDefaultMultiplayerRoom].playersInRoom[connection]
                .variables.guestNameOfSocketOwner
        );
      }
      ...
      data.currentGame.updateRound =
        rooms[roomIDOfDefaultMultiplayerRoom].updateRound;
...
function constructMinifiedGameDataObjectToSend(connectionID, playerIndex) {
  ...
  minifiedOpponentGameData.enemies = minifyEnemies(
    rooms[roomIDOfDefaultMultiplayerRoom].data.currentGame.players[connectionID]
      .currentGame.enemiesOnField
  );
...
uWS
  ...
  .ws("/", {
      ...
      switch (parsedMessage.action) {
        case "createAndJoinDefaultSingleplayerRoom": {
          ...
          if (socket.variables.currentRoomSocketIsIn === "") {
            if (
              parsedMessage.arguments.gameMode == "easySingleplayerMode" ||
              parsedMessage.arguments.gameMode == "standardSingleplayerMode"
            ) {
        ...
        case "keypress": {
          ...
          // validation 1
          if (
            utilities.checkIfVariablesAreUndefined(code, playerTileKeybinds)
          ) {
            return;
          }
              ...
              if (code != "Escape") {
                switch (rooms[socket.variables.currentRoomSocketIsIn].type) {
                  case modes.SINGLEPLAYER: {
                    if (
                      socket.variables.currentRoomSocketIsIn != "" &&
                      socket.variables.socketIsHostOfRoomItIsIn
                    ) {
                  ...
                  case modes.DEFAULT_MULTIPLAYER: {
                    if (
                      socket.variables.currentRoomSocketIsIn != "" &&
                      socket.variables.ownerOfSocketIsPlaying
                    ) {
                      tiles.forceSelectTileWithTermID(
                        tiles.convertPressedKeyToTermID(
                          code,
        ...
        case "leaveRoom": {
          ...
          if (
            ...
            if (
              ...
              rooms[roomToLeave].data.currentGame.players[socket.connectionID]
            ) {
        ...
        case "chatMessage": {
          if (
            /\S/.test(parsedMessage.arguments.message.toString()) &&
            ...
            broadcastToEverySocketInRoom(
              ...
              JSON.stringify({
                ...
                arguments: {
                  ...
                  )}">${utilities.getNameOfSocketOwner(socket)}</span>: ${
                    parsedMessage.arguments.message
                  }</div>`,
                  useHTML: true
                }
            ...
            JSON.stringify({
              ...
              arguments: {
                ...
                message: DOMPurify.sanitize(
                  ...
                  } ${utilities.getNameOfSocketOwner(socket)}:<br>${
                    parsedMessage.arguments.message
                  }`
                )
              }
        ...
        case "sendReport": {
          ...
          let result = await moderation.sendReport(
            socket,
            parsedMessage.arguments.reportTarget,
            parsedMessage.arguments.reportDescription
          );
          let message = "";
          if (result) {
          ...
          socket.send(
            JSON.stringify({
              action: "createToastNotification",
              arguments: { message: DOMPurify.sanitize(message) }
            })
          );
          break;
        }
    ...
    close: (socket, code, message) => {
      deleteSocket(
        global.sockets.find(
          (socketToClose) => socketToClose.connectionID === socket.connectionID
        )
      );
    },
