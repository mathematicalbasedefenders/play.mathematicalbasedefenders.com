...
socket.onerror = (event) => {
  alert(
    "Disconnected from server. Click OK to refresh page. Error Code: " + event
  );
  location.reload();
};
      ...
      switch (currentGameData.currentGame.mode) {
        case "singleplayer": {
          if (currentGameData.currentGame.gameIsOver) {
            ...
            for (let i = 0; i < 49; i++) {
              ...
              if (currentGameData.currentGame.tilesOnBoard[i]) {
                ...
                game.tilesOnBoard[i].sprite.texture =
                  tileTextures[
                    currentGameData.currentGame.tilesOnBoard[i].selected ? 1 : 0
                  ][
                    currentGameData.currentGame.tilesOnBoard[i].termID == 12 &&
                    settings.video.multiplicationSignForm == "dot"
                      ? 23
                      : currentGameData.currentGame.tilesOnBoard[i].termID
                  ];
            ...
            for (
              let i = 0;
              i < currentGameData.currentGame.enemiesOnField.length;
              i++
            ) {
              ...
              if (enemy !== undefined && enemy !== null && !enemy.toDestroy) {
                if (
                  ...
                  game.enemyRenderStatus[enemy.enemyNumber.toString()][
                    "enemySprite"
                  ].enemyNumber = enemyObject.enemyNumber.toString();
                  game.enemyRenderStatus[enemy.enemyNumber.toString()][
                ...
                game.enemyRenderStatus[enemy.enemyNumber.toString()][
                  "enemySprite"
                ].x = initialWindowWidth / 2 + 80 * (enemy.sPosition - 5); // (enemy.sPosition / 10) * 800 + 560;
            ...
            for (
              let i = 0;
              i < currentGameData.currentGame.scoreGainIndicators.length;
              i++
            ) {
              let indicator =
              ...
              if (
                ...
                if (
                  ...
                  let scoreGainIndicator = new PIXI.Text(
                    currentGameData.currentGame.scoreGainIndicators[i].content,
                  ...
                  game.scoreGainIndicatorRenderStatus[
                    indicator.number.toString()
                  ]["requestedValueTextSprite"] = scoreGainIndicator;
                  game.scoreGainIndicatorRenderStatus[
                    indicator.number.toString()
                  ]["rendered"] = true;
            ...
            for (let numberToRemoveAsString of renderedEnemiesOnFieldToDelete) {
              game.enemyRenderStatus[numberToRemoveAsString.toString()] ===
                undefined ||
                singleplayerScreenContainer.removeChild(
                  game.enemyRenderStatus[numberToRemoveAsString.toString()][
                    "enemySprite"
        ...
        case "defaultMultiplayerMode": {
          {
            ...
            if (currentGameData.currentGame.dead) {
              ...
              if (game.opponentGameInstances.length == 0) {
                // opponents
                for (let opponentGameData of currentGameData.currentGame
                  .opponentGameData) {
                  game.opponentGameInstances.push(
                    new OpponentGameInstance(opponentGameData)
                  );
                ...
                for (let opponentGameData of currentGameData.currentGame
                  .opponentGameData) {
                  let opponentGameInstance = game.opponentGameInstances.filter(
                ...
                if (
                  currentGameData.currentGame.opponentGameData.length !=
                    game.cachedLengthOfOpponentGameInstances ||
                  ...
                  for (let opponentGameData of currentGameData.currentGame
                    .opponentGameData) {
                    let instance = game.opponentGameInstances.filter(
                      (opponentGameInstance) => {
                        return (
                          opponentGameInstance &&
                          opponentGameInstance.playerIndex ==
              ...
              multiplayerScreenContainerItems.currentComboTimeLeftText.text =
                ...
                currentGameData.currentGame.currentCombo < 1 ||
                currentGameData.currentGame
                  .timeElapsedSinceLastEnemyKillInMilliseconds > 5000
                  ? ""
                  : turnMillisecondsToTime(
                      5000 -
                        currentGameData.currentGame
                ...
                if (currentGameData.currentGame.tilesOnBoard[i]) {
                  ...
                  game.tilesOnBoard[i].sprite.texture =
                    tileTextures[
                      ...
                      currentGameData.currentGame.tilesOnBoard[i].termID ==
                        12 && settings.video.multiplicationSignForm == "dot"
                        ? 23
                        : currentGameData.currentGame.tilesOnBoard[i].termID
                    ];
              ...
              for (
                ...
                if (enemy !== undefined && enemy !== null && !enemy.toDestroy) {
                  ...
                  game.enemyRenderStatus[enemy.enemyNumber.toString()][
                    "enemySprite"
                  ].x = initialWindowWidth / 2 + 80 * (enemy.sPosition - 5); // (enemyObject.sPosition / 10) * 800 + 560;
                  game.enemyRenderStatus[enemy.enemyNumber.toString()][
                    "enemySprite"
                  ].y = enemy.yPosition;
                  game.enemyRenderStatus[enemy.enemyNumber.toString()][
              ...
              for (
                let i = 0;
                i < currentGameData.currentGame.enemiesSentIndicators.length;
                i++
              ) {
                let indicator =
                ...
                if (
                  ...
                  if (
                    ...
                    game.enemiesSentIndicatorRenderStatus[
                      indicator.number.toString()
                    ]["requestedValueTextSprite"] = enemiesSentIndicator;
                    game.enemiesSentIndicatorRenderStatus[
                      indicator.number.toString()
                    ]["rendered"] = true;
                    // game.spritesOfRenderedEnemiesOnField.push(enemySprite);
