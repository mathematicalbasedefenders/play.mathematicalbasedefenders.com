...
function createToastNotification(options) {
  for (let toastNotification in game.toastNotifications) {
    if (
      game.toastNotifications[toastNotification].zonePosition == "bottomRight"
    ) {
      $(
  ...
  $("#toast-notifications-container").append(
    `<div id="toast-notification-${game.toastNotificationsCreated}-container" ${
      options.position?.toString()?.indexOf("Center") > -1
        ? 'style="display:flex;justify-content:center;left:50%;right:auto"'
        : (style = "")
