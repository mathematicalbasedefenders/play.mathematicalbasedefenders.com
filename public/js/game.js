...
function setMultiplayerScreenItemOffsets(xOffset, yOffset) {
  ...
  multiplayerScreenContainerItems.sendButtonSprite.on(
    "pointerdown",
    function () {
      sendProblem();
    }
  ...
  multiplayerScreenContainerItems.baseSprite.x =
    initialWindowWidth / 2 - 450 + xOffset;
  ...
  multiplayerScreenContainerItems.baseHealthText.x =
    initialWindowWidth / 2 + 64 * 2 + 96 + xOffset;
  multiplayerScreenContainerItems.baseHealthText.y =
    initialWindowHeight / 2 + 60 + yOffset;
  ...
  multiplayerScreenContainerItems.valueOfVariableDText.y =
    initialWindowHeight / 2 + 210 + yOffset;
  ...
  multiplayerScreenContainerItems.currentPlayersRemainingText.y =
    initialWindowHeight / 2 + 240 + yOffset;
...
if (isMobile) {
  showTextModal(
    "It looks like you are playing on a phone or tablet. Please note that the game might not be fully playable on a phone or tablet.",
    "Note"
  );
}
  ...
  switch (currentScreen) {
    ...
    case screens.MULTIPLAYER_GAME_SCREEN: {
      ...
      multiplayerScreenContainerItems.playerNameText.x =
        (initialWindowWidth -
          PIXI.TextMetrics.measureText(
            multiplayerScreenContainerItems.playerNameText.text === undefined
              ? ""
...
function setPropertiesAndChangeSettingsScreen(newSettingsScreen) {
  ...
  switch (newSettingsScreen) {
    ...
    case settingsScreens.AUDIO_SETTINGS_SCREEN: {
      document.getElementById("audio-settings-screen-container").style.display =
        "block";
      break;
    }
...
function updateSettingsSelections() {
  ...
  $("#game-screen-information-mode-setting-drop-down-menu").val(
    settings.video.gameScreenInformationMode
  );
  // input
  for (let i = 0; i < 19; i++) {
...
function testCustomEnemyPicture(url) {
  ...
  if (document.querySelector("#picture-tester")) {
    if (
      !document.querySelector("#picture-tester").querySelector("img").complete
    ) {
...
function authenticate() {
  ...
  formData.append(
    "username",
    $("#login-form").find("#username-input-box").val()
  );
  formData.append(
    "password",
    $("#login-form").find("#password-input-box").val()
  );
  $.ajax({
    url: `/authenticate`,
    method: "POST",
...
function setLabelerTextVisibilities() {
  ...
  if (
    isNaN(settings.video.gameScreenInformationMode) ||
    !settings.video.gameScreenInformationMode
  ) {
