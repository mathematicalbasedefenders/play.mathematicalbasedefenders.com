...
function createToastNotification(options) {
	...
	$("#toast-notifications-container").append(
		`<div id="toast-notification-${game.toastNotificationsCreated}" class="toast-notification ${(options.position || "bottomRight").replace(/([A-Z])/, "-$1").toLowerCase()}-toast-notification" style="border-color:${
	...
	game.toastNotifications[game.toastNotificationsCreated].zonePosition = options.position;
	game.toastNotifications[game.toastNotificationsCreated].number = game.toastNotificationsCreated;
	game.toastNotifications[game.toastNotificationsCreated].expiryTime = Date.now() + 5000;
}
