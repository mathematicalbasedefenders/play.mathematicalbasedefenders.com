...
describe("MultiplayerRoom", () => {
  ...
  it("should allow joining custom multiplayer room once the message is sent, and the room exists", async () => {
    ...
    try {
      await Promise.all([
        new Promise((resolve) => socket1.addEventListener("open", resolve)),
        new Promise((resolve) => socket2.addEventListener("open", resolve))
      ]);
      ...
      await waitForWebSocketMessage(socket1, (data: { [key: string]: any }) => {
        return (
          ...
          data.data.message.indexOf(
            "Successfully set room's constant property"
          ) > -1
        );
      });
